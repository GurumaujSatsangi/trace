...
app.post("/api/sar/download", express.json(), (req, res) => {
    try {
        const { transaction, verdict } = req.body;
        ...
        res.setHeader('Content-Disposition', `attachment; filename="SAR_${transaction.transaction_id || 'Report'}.pdf"`);
        
        doc.pipe(res);

        doc.fontSize(20).font('Helvetica-Bold').text("SUSPICIOUS ACTIVITY REPORT (SAR)", { align: 'center' });
        doc.moveDown(0.5);
        doc.fontSize(10).font('Helvetica').fillColor('gray').text(`Generated by TRACE AI Compliance System | ${new Date().toLocaleString()}`, { align: 'center' });
        doc.moveDown(2);

        doc.fontSize(14).font('Helvetica-Bold').fillColor('black').text("1. Transaction Details");
        doc.moveDown(0.5);
        ...
        const details = [
            ...
            `Risk Level: ${transaction.risk_level || 'N/A'}`,
            `Risk Score: ${transaction.risk_score || 'N/A'}`
        ];
        ...
        doc.fontSize(14).font('Helvetica-Bold').text("2. Compliance Verdict");
        doc.moveDown(0.5);
        doc.fontSize(10).font('Helvetica');
        
        if (verdict) {
            const cleanVerdict = verdict.replace(/<[^>]*>?/gm, '');
            doc.text(cleanVerdict, { align: 'justify', indent: 20 });
        } else {
            doc.text("No verdict provided.", { indent: 20 });
        }
        
        doc.moveDown();

        doc.fontSize(14).font('Helvetica-Bold').text("3. Adverse Media & Flags");
        doc.moveDown(0.5);
        doc.fontSize(10).font('Helvetica');
        
        if (transaction.flags && transaction.flags.length > 0) {
            doc.font('Helvetica-Bold').text("Risk Flags:", { indent: 20 });
            doc.font('Helvetica');
            let flagArray = transaction.flags;
            if (typeof flagArray === 'string') {
                try { flagArray = JSON.parse(flagArray); } catch(e) {}
            }
            if (Array.isArray(flagArray)) {
                flagArray.forEach(flag => {
                    doc.text(`• ${flag}`, { indent: 30 });
                });
            }
            doc.moveDown(0.5);
        }

        if (transaction.adverse_media_links) {
            doc.font('Helvetica-Bold').text("Relevant Links:", { indent: 20 });
            doc.font('Helvetica').fillColor('blue');
            let linksArray = transaction.adverse_media_links;
            if (typeof linksArray === 'string') {
                try { linksArray = JSON.parse(linksArray); } catch(e) {}
            }
            if (Array.isArray(linksArray)) {
                linksArray.forEach(link => {
                    doc.text(`• ${link}`, { indent: 30, link: link, underline: true });
                });
            }
        } else {
            doc.fillColor('black').text("No adverse media links found.", { indent: 20 });
        }
        ...
        res.status(500).json({ error: "Failed to generate SAR PDF" });
    }
});
